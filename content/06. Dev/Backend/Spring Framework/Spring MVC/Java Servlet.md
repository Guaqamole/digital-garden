...
> [!question]  HTTP 프로토콜을 이용한 서버와 클라이언트의 통신 과정은?
...
> 
> 서버는 파이프라인을 통해 CGI 프로그램에서 출력한 결과 페이지의 데이터를 읽어, HTTP 응답헤더를 생성하여 데이터를 함께 반환해줍니다.
...
## Servlet Container 정의
클라이언트의 요청(Request)을 받아주고 응답(Response)할 수 있게, 웹서버와 소켓으로 통신하고 서블릿을 관리해주는 컨테이너 ex) apache tomcat
