---
title: Deep Learning 공부순서
date: 2025-09-09
draft: false
tags:
  - DL
complete: true
banner: /_assets/_img/_banners/nebula.jpg
---
Step 5) Deep Learning
- [Deep Learning Tutorial for Beginners](https://www.kaggle.com/kanncaa1/deep-learning-tutorial-for-beginners)
- [Pytorch Tutorial for Deep Learning Lovers](https://www.kaggle.com/kanncaa1/pytorch-tutorial-for-deep-learning-lovers)
- [Introduction to CNN Keras - 0.997 (top 6%)](https://www.kaggle.com/yassineghouzam/introduction-to-cnn-keras-0-997-top-6)
- [Zero to Hero in Pytorch(ML+DL+RL)](https://www.kaggle.com/ashishpatel26/zero-to-hero-in-pytorch-ml-dl-rl)
- [Welcome to deep learning (CNN 99%)](https://www.kaggle.com/toregil/welcome-to-deep-learning-cnn-99)
- [MNIST with Keras for Beginners(.99457)](https://www.kaggle.com/adityaecdrid/mnist-with-keras-for-beginners-99457)
- [Recurrent Neural Network with Pytorch](https://www.kaggle.com/kanncaa1/recurrent-neural-network-with-pytorch)

prerequisite (pandas + numpy)
- https://www.kaggle.com/discussions/getting-started/123146
- https://www.kaggle.com/code/themlphdstudent/learn-numpy-numpy-50-exercises-and-solution
- https://www.programiz.com/python-programming/numpy/getting-started
- https://www.kaggle.com/code/abhi8923shriv/pandas-library-80-exercises

books
- 친절한 딥러닝 수학
	- Ch.5 선형대수 1부
	- Ch.6 선형대수 2부
		- 행렬식
		- PCA
	- Ch.2 확률 1부
	- Ch.3 확률 2부
	- Ch.4 통계
		- 
- 딥러닝을 위한 수학
	- Ch.2 ~
- 신경망첫걸음
	- 개념
		- Ch.7 ~ 16 (p.140)
	- 실습
		- Ch.20 파이썬으로 인공신경망 만들기 ⭐
		- Ch. 21 Minist 손글씨 데이터 인식하기
- 텐초의 파이토치 딥러닝
	- 개념
		- Ch1. 딥러닝 문제해결 프로세스, 체크리스트, 최소한 통계 개념
		- Ch.2~ Ch.3 
	- 실습
		- Ch.10 LSTM 텍스트 생성
		- Ch.11 어텐션 기계 번역
		- Ch.12 CRNN + GRU
- 딥러닝 쿡북
	- Ch.2 문제 해결하기 - 활성화 함수 고르기, 네트워크 구조, 배치 크기, 학습속도

blog
- https://sonstory.tistory.com/category/Deep%20Learning
- https://huangdi.tistory.com/category/Computer%20Science/Deep%20Learning?page=3 -> *비전공자용*